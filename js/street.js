...
function onSearchRue(event) {
    ...
    fetch(`./search.php?rue=${value}`).then(function (response) {
...
function onSearchCategorie(event) {
    ...
    // console.log(value);
    const streetSection = document.querySelector('#galerie');
    fetch(`./search.php?categorie=${value}`).then(function (response) {
        // console.log(response);
        //streetSection.innerHTML = response;
        return response.text();
    }).then(function (streets) {
...
function onSearchKm(event) {
    ...
    const streetSection = document.querySelector('#galerie');
    fetch(`./search.php?km=${value}&latitude=${valueLat}&longitude=${valueLon}`).then(function (response) {
        // console.log(response);
        //streetSection.innerHTML = response;
        return response.text();
    }).then(function (streets) {
...
function localise(event) {
    ...
    if ("geolocation" in navigator) {
        let watchID = navigator.geolocation.getCurrentPosition(function (position) {
            ...
            fetch(`https://api-adresse.data.gouv.fr/reverse/?lon=` + position.coords.longitude + `&lat=` + position.coords.latitude + `&type=housenumber&lat=45.75&lon=4.85`).then(function (response) {
                return response.json();
            }).then(data => {
